#!/bin/env python
#
# Outputs a text file suitable for consumption via Palo Alto EDL
...
class PaloAltoEDL(Base, OutputInterface):
    __rule_list = dict()
    __ext = 'config'
    __target_file_path = ''
    ...
    def run(self) -> bool:
        """
        Output the ACL's in 'rule_list' to the 'target_file_path' in Palo Alto EDL format
        """
        ...
        self.info(f"Writing Palo Alto EDL file to: '{self.__target_file_path}'")
        ...
        with open(self.__target_file_path, mode='w') as target_file_handle:
            ...
            for acl_list_name in self.__rule_list:
                for destination in self.__rule_list[acl_list_name]:
                    ...
                    print(destination, file=target_file_handle, end=linesep)
